...
class QuizzBrain{
  ...
  String getNextQuestion(int index){
    if(isGameOver(index))
      return 'Result: ' + _correct.toString() + '/13';
    else{
      return getQuestionText(index);
    }
  }


  void checkAnswer(bool userAnswer, int index){
    if(userAnswer == getQuestionAnswer(index)){
      _correct++;
  ...
  bool isGameOver(int index){
    return (index > getQuestions().length-1);
  }
